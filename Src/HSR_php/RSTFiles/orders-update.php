...
if(isset($_SESSION['cart'])) {
  ...
  foreach($_SESSION['cart'] as $product_id => $quantity) {
    ...
    $result = $mysqli->query("SELECT * FROM products WHERE order id = ".$product_id);
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        $query = $mysqli->query("INSERT INTO orders (id, Status, product_code, product_name, product_desc, price, units, total, email) VALUES('$obj->S.No','$obj->Order ID','$obj->Status','$obj->product_code', '$obj->product_name', '$obj->product_desc', $obj->price, $quantity, $cost, '$user')");
        ...
        if($query){
          while ($obj = $query->fetch_object()){
            ...
            $message = "<html><body>";
			$message .= '<p><strong>S.No</strong>: '.$obj->s.no.'</p>';
            $message .= '<p><h4>Order ID ->'.$obj->id.'</h4></p>';
			$message .= '<p><strong>Status update</strong>: '.$obj->status.'</p>';
